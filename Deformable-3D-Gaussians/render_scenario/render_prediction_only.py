...
import torch
from scene import Scene, DeformModel, DeformPredict
import os
...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, deform_predict):
    ...
    print('NEW RENDERING 04/06/2024 ver 2')
    frame_number = 0
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, "train", scene.loaded_iter,
                        ...
                        background, deform,deform_predict)
