...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    print('PLATE-13 TIME INTERVAL OF 5')
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        ### Apply Prediction after Prediction Iterations 
            ...
            with torch.no_grad():
                ...
                # Render
                render_pkg_re_before = render(viewpoint_cam, gaussians, pipe, background, d_xyz_before, d_rotation_before, d_scaling_before, dataset.is_6dof)
                image_before, viewspace_point_tensor_before, visibility_filter_before, radii_before = render_pkg_re_before["render"], render_pkg_re_before[
                    "viewspace_points"], render_pkg_re_before["visibility_filter"], render_pkg_re_before["radii"]
                ...
                # Render
                render_pkg_re_after = render(viewpoint_cam, gaussians, pipe, background, d_xyz_after, d_rotation_after, d_scaling_after, dataset.is_6dof)
                ...
                ## Diff xyz
                #print('Size', gaussians.get_xyz.detach().size())
                #print('Size d_xyz_before', d_xyz_before.size())
                #print('Size diff', (d_xyz_before - d_xyz_after).size())
                #print('SUM DIFF > 0: ',torch.sum(d_xyz_before - d_xyz_after > 0))
                #print('SUM DIFF < 0: ',torch.sum(d_xyz_before - d_xyz_after < 0))
                #print('SUM DIFF = 0: ',torch.sum(d_xyz_before - d_xyz_after == 0))
                #print((d_xyz_before - d_xyz_after))
            ...
            lambda_motion = 0.2 + 0.1*iteration/opt.iterations
            # overall loss
...
def training_report_with_prediction(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                    renderArgs, deform, deform_predict, load2gpu_on_the_fly, is_6dof=False, h = 5): # <------------------------------------------ UPDATED
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('iter_time', elapsed, iteration)
