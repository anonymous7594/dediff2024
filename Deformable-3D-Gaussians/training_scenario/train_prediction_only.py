...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    print('NEW TRAINING 04/06/2024 ver 1')
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        total_frame_tracking = total_frame - frame_number
        ...
        if iteration < opt.warm_up:
            ...
            actual_motion = d_xyz_after - d_xyz_before
            pred_motion = (d_xyz_after - d_xyz_pred) + (d_xyz_pred - d_xyz_before)
            motion_loss = l2_loss(pred_motion, actual_motion)
            lambda_motion = 0.2

            loss = (1.0 - opt.lambda_dssim - lambda_motion) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + lambda_motion*motion_loss
            loss.backward()
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    d_xyz_before, d_rotation_before, d_scaling_before = deform.step(xyz.detach(), rotation.detach(), scaling.detach(), time_input_before)
                    d_xyz_after, d_rotation_after, d_scaling_after = deform.step(xyz.detach(), rotation.detach(), scaling.detach(), time_input_after)
