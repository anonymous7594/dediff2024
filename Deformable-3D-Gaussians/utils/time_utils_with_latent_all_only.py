...
class Deform_Predict(nn.Module):
    def __init__(self, D=4, W=32, Dd=4, Wd=32, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False
                 ...
                 ,input_image_bedding=768, output_image_embedding=1280, Wi = 64, Ddi = 4 # MLP to decode latent compilation from Diffusion Model
                 ):
    ...
    #def forward(self, time_input, xyz, new_feature_latent_data):    
        ...
        # Before
        d_xyz_before_dynamic = d_xyz_before #d_xyz_before[condition].to(device) #d_xyz_before.to(device) #
        #d_xyz_before_static = d_xyz_before[~condition].to(device)
        time_input_before = time_input_before#[condition].to(device)
        # After
        d_xyz_after_dynamic = d_xyz_after #d_xyz_after[condition].to(device)
        ...
        if new_feature_latent_data.numel() == 0:
            ...
            if new_feature_latent_data_all == []:
                ...
                new_feature_latent_data_all = self.decoder_latent_compilation(new_feature_latent_data_all+time_input[0,:])
                # Flatten the tensor to 1D
        ...
        ## combine value
        d_xyz_combined = d_xyz #torch.cat((d_xyz, dxyz_remaining), dim=0)
        ...
        ## combine value
        d_rotation_combined = d_rotation #torch.cat((d_rotation, d_rotation_remaining), dim=0)
        ...
        ## combine value
        d_scaling_combined = d_scaling #torch.cat((d_scaling, d_scaling_remaining), dim=0)
