...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False): #D=8
        ...
        ## embedding rotation
        self.embed_nf_r, xyz_input_ch_r = get_embedder(multires, 4)
        self.input_ch_r = xyz_input_ch + time_input_ch + xyz_input_ch_r

        ## embedding scaling
        self.embed_nf_s, xyz_input_ch_s = get_embedder(multires, 3)
        self.input_ch_s = xyz_input_ch + time_input_ch + xyz_input_ch_s
        ...
        if is_blender:
            ...
            #### Encoder-Decoder ####
            ## d_xyz
            self.encoder_dxyz = nn.Sequential(
            nn.Linear(self.input_ch, 64),
            nn.ReLU(),
            nn.Linear(64, 128),
            nn.ReLU()
            )
            self.decoder_dxyz = nn.Sequential(
                nn.Linear(128, 64),
                nn.ReLU(),
                nn.Linear(64, 3)  # Predict x, y, z at time t
            )
            ## roration
            self.encoder_rotation = nn.Sequential(
            nn.Linear(self.input_ch_r, 64),
            nn.ReLU(),
            nn.Linear(64, 128),
            nn.ReLU()
            )
            self.decoder_rotation = nn.Sequential(
                nn.Linear(128, 64),
                nn.ReLU(),
                nn.Linear(64, 4)  
            )
            ## d_scaling
            self.encoder_scaling = nn.Sequential(
            nn.Linear(self.input_ch_s, 64),
            nn.ReLU(),
            nn.Linear(64, 128),
            nn.ReLU()
            )
            self.decoder_scaling = nn.Sequential(
                nn.Linear(128, 64),
                nn.ReLU(),
                nn.Linear(64, 3) 
            )
    ...
    def forward(self, x, r, s, t):
        t_emb = self.embed_time_fn(t)
        if self.is_blender:
            t_emb = self.timenet(t_emb)  # better for D-NeRF Dataset
        x_emb = self.embed_fn(x)
        r_emb = self.embed_nf_r(r)
        s_emb = self.embed_nf_s(s)

        h = torch.cat([x_emb, t_emb], dim=-1)
        h_r = torch.cat([x_emb, r_emb, t_emb], dim=-1)
        h_s = torch.cat([x_emb, s_emb, t_emb], dim=-1)
        #### ORIGINAL CODE
        ...
        #### Encoder-Decoder ####
        dxyz_init = self.encoder_dxyz(h)
        d_xyz = self.decoder_dxyz(dxyz_init)

        rotation_init = self.encoder_rotation(h_r)
        rotation = self.decoder_rotation(rotation_init)

        scaling_init = self.encoder_scaling(h_s)
        scaling = self.decoder_scaling(scaling_init)
        #### -------------
...
class Deform_Predict(nn.Module):
    def __init__(self, D=4, W=128, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False): #D=8, W=256
