...
class DeformNetwork(nn.Module):
    def __init__(self, D=16, W=512, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False):
        ...
        self.embed_time_fn_0, time_input_ch_0 = get_embedder(self.t_multires, 1)
        self.input_ch = xyz_input_ch + time_input_ch + time_input_ch_0
    ...
    def forward(self, x, t, time_input_0):
        t_emb = self.embed_time_fn(t)
        t_emb_0 = self.embed_time_fn_0(time_input_0)
        if self.is_blender:
            t_emb = self.timenet(t_emb)  # better for D-NeRF Dataset
        x_emb = self.embed_fn(x)
        h = torch.cat([x_emb, t_emb, t_emb_0], dim=-1)
        for i, l in enumerate(self.linear):
            h = self.linear[i](h)
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([x_emb, t_emb, t_emb_0, h], -1)
