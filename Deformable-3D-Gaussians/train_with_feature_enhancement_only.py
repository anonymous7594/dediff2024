...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        ### STAGE 2: Apply deform() and train to get latent data for feature enhancement
            ...
            #time_dim_before = time_input_before[0,:]
            #time_dim_after = time_input_after[0,:]
            new_feature_latent_data = feature_enhancement.step(features_before,features_after,time_dim)
            #new_feature_latent_data_current = new_feature_latent_data_current.unsqueeze(0)
            #new_feature_latent_data_past = feature_enhancement.step(features_before,features_after,time_dim_before)
            #new_feature_latent_data_past = new_feature_latent_data_past.unsqueeze(0)
            #new_feature_latent_data_future = feature_enhancement.step(features_before,features_after,time_dim_after)
            #new_feature_latent_data_future = new_feature_latent_data_future.unsqueeze(0)
            #print(new_feature_latent_data.dtype)
            #print(new_feature_latent_data.size())
            #new_feature_latent_data = torch.cat([new_feature_latent_data_current, new_feature_latent_data_past, new_feature_latent_data_future], 0) 
            new_feature_latent_data = new_feature_latent_data.to(device)
            #print(latent_compiled.size())
            #print(latent_compiled.dtype)
            #print(new_feature_latent_data.size())
            #print(new_feature_latent_data.dtype)
            ...
            if k_th + 1 < len(key_frame_list):
                ...
                k_th = 0
            '''
            ### Apply latent data on all frames only
            ...
            #with torch.no_grad():
            #    d_xyz_after, d_rotation_after, d_scaling_after, h_input = deform.step(gaussians.get_xyz.detach(),time_input+ast_noise)
            #d_xyz, d_rotation, d_scaling = deform_predict.step(time_input+ast_noise, gaussians.get_xyz.detach(), new_feature_latent_data)
            #d_xyz, d_rotation, d_scaling = deform_predict.step(time_input+ast_noise, h_input, new_feature_latent_data)
            # Render
            render_pkg_re = render(viewpoint_cam, gaussians, pipe, background, d_xyz, d_rotation, d_scaling, dataset.is_6dof)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg_re["render"], render_pkg_re[
                    "viewspace_points"], render_pkg_re["visibility_filter"], render_pkg_re["radii"]
            ...
            with torch.no_grad():
                ...
                if iteration > len(key_frame_list)*10*m_1 + total_frame: 
                    cur_psnr = training_report_with_prediction(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                                            testing_iterations, scene, render, (pipe, background), deform, deform_predict, feature_enhancement,
...
def training_report_with_prediction(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                    renderArgs, deform, deform_predict, feature_enhancement, load2gpu_on_the_fly, is_6dof=False, h = 5, using_latent=True, pretrain_latent=True): #UPDATED: h, using_latent, pretrain_latent
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('iter_time', elapsed, iteration)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    '''
                    ## Rendering the image data from key frames -> Saving image
                    image_before = render(viewpoint_cam_before, scene.gaussians, *renderArgs, d_xyz_before, d_rotation_before, d_scaling_before, is_6dof)["render"].to(device)
                    image_after = render(viewpoint_cam_after, scene.gaussians, *renderArgs, d_xyz_after, d_rotation_after, d_scaling_after, is_6dof)["render"].to(device)                
                    features_before = image_before.to(device)
                    features_after = image_after.to(device)
                    time_dim = time_input[0,:]
                    new_feature_latent_data = feature_enhancement.step(features_before,features_after,time_dim)
