...
from utils.loss_utils import l1_loss, ssim, kl_divergence, l2_loss, calculate_motion_loss, calculate_rigid_loss_knn
...
def pick_random_number_less_than(lst, num):
...
### Pick up a random number
def pick_random_number_more_than(lst, num):
    filtered_list = [x for x in lst if x > num]
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for i in number_of_frames:
        if (i == 0) or (i % 4 == 0) or (i == len(number_of_frames)-1):
            key_frame_list.append(i)
    key_frame_list_in_loop = key_frame_list.copy()

    print('NEW TRAINING 18/06/2024 WITH AS-77')
    ...
    for iteration in range(1, opt.iterations + 1):
        if iteration <= iteration_predict:
            ...
            #print('fid: ',fid)
            #print('key_frame_before: ',key_frame_before,' fid_before: ',fid_before)
            #print('key_frame_after: ',key_frame_after,' fid_after: ',fid_after)
            ...
            if iteration < opt.warm_up:
                ...
                #with torch.no_grad():
                d_xyz, d_rotation, d_scaling = deform.step(gaussians.get_xyz.detach(), 
                                                                                #gaussians.get_rotation.detach(),
                                                                                #gaussians.get_scaling.detach(),
                                                                                time_input + ast_noise)
                        
                #d_xyz_after, d_rotation_after, d_scaling_after = deform.step(gaussians.get_xyz.detach(),
                                                                            #gaussians.get_rotation.detach(),
                                                                            #gaussians.get_scaling.detach(),
                                                                            #time_input_after)
                ...
                # Predict
                #d_xyz, d_rotation, d_scaling = deform_predict.step(time_input + ast_noise,time_input_before,time_input_after,d_xyz_before, d_xyz_after)
            ...
            if iteration < opt.warm_up:
                ...
                # rigid Loss
                #d_xyz_pred = gaussians.get_xyz.detach() + d_xyz
                #rigid_loss = calculate_rigid_loss_knn(d_xyz_before[abs(d_xyz)>0], d_xyz_pred[abs(d_xyz)>0], d_xyz_after[abs(d_xyz)>0],5)
                lambda_rigid = 0
                # overall loss
                loss = (1.0 - opt.lambda_dssim - lambda_motion - lambda_rigid) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) 
                #loss = loss + lambda_motion*motion_loss 
                #loss = loss + lambda_rigid*rigid_loss
                loss.backward()
        ...
        ### Apply Prediction after Prediction Iterations 
            ...
            # Previous key frame
            # Nearest
            key_frame_before =  (frame_number//4)*4 # every h-th frame
            viewpoint_cam_before = viewpoint_stack[key_frame_before]
            # Random
            #if key_frame_before == 0:
            #    random_key_frame_before = 0
            #else: 
            #    random_key_frame_before = pick_random_number_less_than(key_frame_list, key_frame_before)
            #viewpoint_cam_before_random = viewpoint_stack[random_key_frame_before]
            # Proceeding key frame
            # Nearest
            key_frame_after =  min((frame_number//4 + 1)*4,total_frame-1) # every h-th frame
            viewpoint_cam_after = viewpoint_stack[key_frame_after]
            # Random
            #if key_frame_after == total_frame-1:
            #    random_key_frame_after = total_frame-1
            #else: 
            #    random_key_frame_after = pick_random_number_more_than(key_frame_list, key_frame_after)
            #viewpoint_cam_after_random = viewpoint_stack[random_key_frame_after]
            ...
            if dataset.load2gpu_on_the_fly:
                ...
                viewpoint_cam_after.load2device()
                # random
                #viewpoint_cam_before_random.load2device()
                #viewpoint_cam_after_random.load2device()
            ...
            fid_after = viewpoint_cam_after.fid
            # random
            #fid_before_random = viewpoint_cam_before_random.fid
            #fid_after_random = viewpoint_cam_after_random.fid
            #print('frame_number: ',frame_number)
            #print('fid: ',fid)
            #print('key_frame_before: ',key_frame_before,' fid_before: ',fid_before)
            #print('key_frame_after: ',key_frame_after,' fid_after: ',fid_after)
            ...
            time_input_after = fid_after.unsqueeze(0).expand(N, -1) 
            # random
            #random_time_input_before = fid_before_random.unsqueeze(0).expand(N, -1) 
            #random_time_input_after = fid_after_random.unsqueeze(0).expand(N, -1) 
            ...
            with torch.no_grad():
                ...
                #print('BEFORE d_xyz: ',d_xyz_before)
                ...
                #print('BEFORE d_xyz_after: ',d_xyz_after)
                d_xyz_after = gaussians.get_xyz.detach() + d_xyz_after
                #d_rotation_after = gaussians.get_rotation.detach() + d_rotation_after
                #d_scaling_after = gaussians.get_scaling.detach() + d_scaling_after
                # Pre-estimated
                ...
                ## Randomness
                # Before
                #random_d_xyz_before, random_d_rotation_before, random_d_scaling_before = deform.step(gaussians.get_xyz.detach(), random_time_input_before)
                #random_d_xyz_before = gaussians.get_xyz.detach() + random_d_xyz_before
                #random_d_rotation_before = gaussians.get_rotation.detach() + random_d_rotation_before
                #random_d_scaling_before = gaussians.get_scaling.detach() + random_d_scaling_before
                # After
                #random_d_xyz_after, random_d_rotation_after, random_d_scaling_after = deform.step(gaussians.get_xyz.detach(),random_time_input_after)
                #random_d_xyz_after = gaussians.get_xyz.detach() + random_d_xyz_after
                #random_d_rotation_after = gaussians.get_rotation.detach() + random_d_rotation_after
                #random_d_scaling_after = gaussians.get_scaling.detach() + random_d_scaling_after
            ...
            if dataset.load2gpu_on_the_fly:
                ...
                viewpoint_cam_after.load2device('cpu')
                # random
                #viewpoint_cam_before_random.load2device('cpu')
                #viewpoint_cam_after_random.load2device('cpu')
            ...
            with torch.no_grad():
                ...
                cur_psnr = training_report_with_prediction(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                                            ...
                                            dataset.load2gpu_on_the_fly, dataset.is_6dof, h = 5)
                if iteration in testing_iterations:
                    if cur_psnr.item() > best_psnr:
                        best_psnr = cur_psnr.item()
                        best_iteration = iteration
...
def training_report_with_prediction(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                    renderArgs, deform, deform_predict, load2gpu_on_the_fly, is_6dof=False, h = 4):
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('iter_time', elapsed, iteration)
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                #key_frame_list = []
                #for i in number_of_frames:
                #    if (i == 0) or (i % 5 == 0) or (i == len(number_of_frames)-1):
                #        key_frame_list.append(i)

                for idx, viewpoint in enumerate(config['cameras']):
                    ## Previous key frame
                    # predetermined
                    key_frame_before =  (frame_number//h)*h #max(frame_number - 1,0) #
                    ...
                    #else: 
                    #    random_key_frame_before = pick_random_number_less_than(key_frame_list, key_frame_before)
                    #viewpoint_cam_before_random = config['cameras'][random_key_frame_before]
                    ## Proceeding key frame
                    # predetermined
                    key_frame_after =  min((frame_number//h + 1)*h,total_frame-1) #min(frame_number + 1,total_frame-1) #
                    ...
                    #else: 
                    #    random_key_frame_after = pick_random_number_more_than(key_frame_list, key_frame_after)
                    #viewpoint_cam_after_random = config['cameras'][random_key_frame_after]
                    ...
                    # random
                    #random_time_input_before = fid_before_random.unsqueeze(0).expand(xyz.shape[0], -1) 
                    #random_time_input_after = fid_after_random.unsqueeze(0).expand(xyz.shape[0], -1) 
                    ...
                    ## Random
                    # Before
                    #random_d_xyz_before, random_d_rotation_before, random_d_scaling_before = deform.step(xyz.detach(), random_time_input_before)
                    # After
                    #random_d_xyz_after, random_d_rotation_after, random_d_scaling_after = deform.step(xyz.detach(),random_time_input_after)
                    ...
                    ## random
                    # before
                    #random_d_xyz_before = xyz + random_d_xyz_before
                    #random_d_rotation_before = xyz + random_d_rotation_before
                    #random_d_scaling_before = xyz + random_d_scaling_before
                    # after
                    #random_d_xyz_after = xyz + random_d_xyz_after
                    #random_d_rotation_after = xyz + random_d_rotation_after
                    #random_d_scaling_after = xyz + random_d_scaling_after
