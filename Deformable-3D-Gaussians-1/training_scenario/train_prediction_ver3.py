...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    frame_number = 0
    print('NEW TRAINING 24/05/2024 ver 2 with Time Interval Update')
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        ## UPDATE: BY FRAME ORDER
        # Scenario: frame number = 0 | last frame | % 5 == 0 (Original code)
        if (frame_number == 0) or (frame_number == (total_frame-1)) or (frame_number % 5 == 0):
            ...
            time_interval = 1 / total_frame_tracking
            #print('TEST FRAME NUMBER: ',frame_number)
            # Load views
            viewpoint_cam = viewpoint_stack[frame_number]
            # First frame
            ...
            if iteration < opt.warm_up:
                ...
                time_input = fid.unsqueeze(0).expand(N, -1) #### Depend on viewpoint_cam
                #time_input_0 = fid_0.unsqueeze(0).expand(N, -1) #### Depend on viewpoint_cam
                #time_input_last = fid_last.unsqueeze(0).expand(N, -1) #### Depend on viewpoint_cam
                time_input_before = fid_before.unsqueeze(0).expand(N, -1) #### Depend on viewpoint_cam
                time_input_after = fid_after.unsqueeze(0).expand(N, -1) #### Depend on viewpoint_cam
                ...
                #d_xyz_0, d_rotation_0, d_scaling_0 = deform.step(gaussians.get_xyz.detach(), time_input_0 + ast_noise)
                #d_xyz_last, d_rotation_last, d_scaling_last = deform.step(gaussians.get_xyz.detach(), time_input_last + ast_noise)
                with torch.no_grad():
                    d_xyz_before, d_rotation_before, d_scaling_before = deform.step(gaussians.get_xyz.detach(), time_input_before + ast_noise)
                    d_xyz_after, d_rotation_after, d_scaling_after = deform.step(gaussians.get_xyz.detach(), time_input_after + ast_noise)
                #d_xyz_0, d_rotation_0, d_scaling_0 = deform.step(gaussians.get_xyz.detach(), time_input_0 + ast_noise)
                #d_xyz_last, d_rotation_last, d_scaling_last = deform.step(gaussians.get_xyz.detach(), time_input_last + ast_noise)

                #print(d_xyz_before)    
                #print(d_rotation_before)
                #print(d_scaling_before)
                ...
                ## MLP approach without embedding
                # Before
                time_diff_before = time_input - time_input_before
                #d_xyz_before = torch.cat([d_xyz_before, time_diff_before], dim=-1)
                #d_rotation_before = torch.cat([d_rotation_before,time_diff_before], dim=-1)
                #d_scaling_before = torch.cat([d_scaling_before,time_diff_before], dim=-1)

                # After
                time_diff_after = time_input_after - time_input
                #d_xyz_after = torch.cat([d_xyz_after, time_diff_after], dim=-1)
                #d_rotation_after = torch.cat([d_rotation_after,time_diff_after], dim=-1)
                #d_scaling_after = torch.cat([d_scaling_after,time_diff_after], dim=-1)

                # Predict
                d_xyz, d_rotation, d_scaling = deform_predict.step(time_diff_before,time_diff_after,
                                                                   d_xyz_before, d_xyz_after,
                                                                   d_rotation_before,d_rotation_after,
                                                                   d_scaling_before,d_scaling_after)

                #print(frame_number,' d_xyz: ',d_xyz.shape)
                #print(frame_number,' d_xyz_before: ',d_xyz_before.shape)
                #print(frame_number,' d_xyz_after: ',d_xyz_after.shape)
                #print(frame_number,' d_xyz_0: ',d_xyz_0.shape)
                #print(frame_number,' d_xyz_last: ',d_xyz_last.shape)
