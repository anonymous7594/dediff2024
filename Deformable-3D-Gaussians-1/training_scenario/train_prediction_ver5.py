...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    frame_number = 0
    print('NEW TRAINING 30/05/2024 ver 1')
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if (frame_number == 0) or (frame_number == (total_frame-1)) or (frame_number % 5 == 0):
            ...
            if iteration < opt.warm_up:
                ...
                d_xyz, d_rotation, d_scaling = deform.step(gaussians.get_xyz.detach(), time_input + ast_noise, time_input_0)
            ...
            if dataset.load2gpu_on_the_fly:
                ...
                viewpoint_cam_0.load2device('cpu')
            ...
            if iteration < opt.warm_up:
                ...
                with torch.no_grad():
                    d_xyz_before, d_rotation_before, d_scaling_before = deform.step(gaussians.get_xyz.detach(), time_input_before, time_input_0)
                    d_xyz_before = gaussians.get_xyz.detach() + d_xyz_before
                    
                d_xyz_after, d_rotation_after, d_scaling_after = deform.step(gaussians.get_xyz.detach(), time_input_after + ast_noise, time_input_0)
            ...
            loss.backward()
                ...
                #alpha = 0.9
                #loss = alpha*loss_current + (1-alpha)*loss_after
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    if (frame_number == 0) or (frame_number == (total_frame-1)) or (frame_number % 5 == 0):
                        viewpoint_cam_0 = config['cameras'][0]
                        ...
                        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input, time_input_0)
                        image = torch.clamp(
                        ...
                        d_xyz_before, d_rotation_before, d_scaling_before = deform.step(xyz.detach(), time_input_before, time_input_0)
                        d_xyz_after, d_rotation_after, d_scaling_after = deform.step(xyz.detach(), time_input_after, time_input_0)
