...
import torch
from scene import Scene, DeformModel, DeformPredict#, Deform_Predict_ver_2#, Deform_Predict_ver_3
import os
...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, deform_predict, using_latent=True): #pretrained_model
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #d_xyz, d_rotation, d_scaling = deform_predict.step(time_input, time_input_before, time_input_after, d_xyz_before, d_xyz_after, new_feature_latent_data) #(features_before, features_after)
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, "train", scene.loaded_iter,
                        ...
                        background, deform,deform_predict) #pretrained_model
